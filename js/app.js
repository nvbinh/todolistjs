  ...
  App.fn.onToggleAll = function(event) {
    ...
    this.stores.findAll(function( items ) {
      $.each( items, function( item ) {
        ...
        self.stores.save( item, function(item) {
          var li = self.getElementByDataId( item.id );
          li.className = item.completed ? STATE_COMPLETED : '';
      ...
      if( filter ) {
        items = items
          .filter(function(item) {
            return item.completed === ( filter === STATE_COMPLETED );
          });
      }
